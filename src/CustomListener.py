...
class CustomListener(ExpressionListener):
    ...
    def set_val(self, ctx: ParserRuleContext):
        ...
        if self.right or (self.parent.rightChild is None and self.parent.leftChild is not None):
            if self.dec_op is not None and isinstance(self.dec_op.leftChild, Array) and self.dec_op.leftChild.pos is not None: # TODO: added this to make complex array's work: int x[i+1] = 1;
