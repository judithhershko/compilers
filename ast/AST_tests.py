import unittest
import AST
import node


class nodeTestCase(unittest.TestCase):
    def test_getId(self):
        testNode = node.AST_node()
        testNode.setLevel(12)
        testNode.setNumber(55)
        self.assertEqual(testNode.getId(), "12.55", "should be 12.55")

    def test_setNodeIds(self):
        ast1 = AST.AST()
        ast2 = AST.AST()

        leaf1 = node.Value(5)
        leaf2 = node.Value(22)
        leaf3 = node.Value(-79)
        mult = node.BinaryOperator("*", leaf1, leaf2)
        neg = node.UnaryOperator("-", leaf3)
        add = node.BinaryOperator("+", mult, neg)
        ast1.setRoot(add)
        ast1.setNodeIds(ast1.root)

        leaf4 = node.Value(5)
        leaf5 = node.Value(22)
        leaf6 = node.Value(-79)
        leaf4.setLevel(2)
        leaf4.setNumber(2)
        leaf5.setLevel(2)
        leaf5.setNumber(3)
        leaf6.setLevel(2)
        leaf6.setNumber(5)
        mult2 = node.BinaryOperator("*", leaf4, leaf5)
        mult2.setLevel(1)
        mult2.setNumber(1)
        neg2 = node.UnaryOperator("-", leaf6)
        neg2.setLevel(1)
        neg2.setNumber(4)
        add2 = node.BinaryOperator("+", mult2, neg2)
        add2.setLevel(0)
        add2.setNumber(0)
        ast2.setRoot(add2)

        self.assertEqual(ast1, ast2, "both ASTs should be the same")

    def test_generateDot(self):
        ast = AST.AST()

        leaf1 = node.Value(5)
        leaf2 = node.Value(22)
        leaf3 = node.Value(-79)
        mult = node.BinaryOperator("*", leaf1, leaf2)
        neg = node.UnaryOperator("-", leaf3)
        add = node.BinaryOperator("+", mult, neg)
        ast.setRoot(add)
        ast.setNodeIds(ast.root)

        dot = ast.generateDot()
        expected = "graph ast {\n0.0 [label=\"Binary operator: +\"]\n1.1 [label=\"Binary operator: *\"]\n2.2 " \
                   "[label=\"Literal: 5\"]\n2.3 [label=\"Literal: 22\"]\n1.4 [label=\"Unary operator: -\"]\n2.5 " \
                   "[label=\"Literal: -79\"]\n\n0.0--1.1\n0.0--1.4\n1.1--2.2\n1.1--2.3\n1.4--2.5\n}"
        self.assertEqual(dot, expected)

if __name__ == '__main__':
    unittest.main()
